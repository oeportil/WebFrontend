export async function getEmpleados() {
  const emp = await fetch(
    `${import.meta.env.VITE_API_URL}/usuarios/ObtenerDash`
  );
  if (emp.status !== 404) {
    return emp.json();
  } else {
...
export async function getTecnicos(tec, email = "") {
  const tecnic = await fetch(
    `${
      import.meta.env.VITE_API_URL
    }/usuarios/ObtenerUsus/${tec}?busqueda=${email}`
  );
...
export async function getTecnicosforTicket(email = "") {
  const tecnic = await fetch(
    `${import.meta.env.VITE_API_URL}/usuarios/ObtenerTecnicos?busqueda=${email}`
  );
  if (tecnic.status !== 404) {
    return tecnic.json();
  } else {
...
export async function createUsuario(user) {
  const usuario = await fetch(
    `${import.meta.env.VITE_API_URL}/usuarios/CrearUsuario`,
    {
      method: "POST",
...
export async function updateUsuario(user, id) {
  console.log(user);
  const usuario = await fetch(
    `${import.meta.env.VITE_API_URL}/usuarios/EditarUsuario/${id}`,
    {
      method: "PATCH",
      body: JSON.stringify(user),
      headers: {
...
export async function getTecnicosByEmail(email = "") {
  const tecnic = await fetch(
    `${import.meta.env.VITE_API_URL}/usuarios/ObtenerUsus/2?busqueda=${email}`
  );
  if (tecnic.status != 404) {
